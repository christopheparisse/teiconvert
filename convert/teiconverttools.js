...
//		console.log(t);
	...
	// Add persons when a code has no name
	for (i in codes) {
		var n = codes[i].name;
		var found = false;
		for (b in names) {
	...
	for (b in lines) {
		if (lines[b].loc === '[+div+]') {
			...
			newXmlId++;
			if (!lines[b].start && !lines[b].end)
				s += '<annotationBlock xml:id="id' + newXmlId + '" who="' + lines[b].loc + '">\n';
			else if (lines[b].start && !lines[b].end)
				s += '<annotationBlock xml:id="id' + newXmlId + '" start="#' + lines[b].start + '" end="" who="' + lines[b].loc + '">\n';
			else if (lines[b].start && !lines[b].end)
				s += '<annotationBlock xml:id="id' + newXmlId + '" start="" end="#' + lines[b].end + '" who="' + lines[b].loc + '">\n';
			else
				s += '<annotationBlock xml:id="id' + newXmlId + '" start="#' + lines[b].start + '" end="#' + lines[b].end + '" who="' + lines[b].loc + '">\n';
...
teiConvertTools.tableToDocx = function(corpus, format, digits) {
	...
	var prevTe = -1;
	if (format.indexOf(';tab;') >= 0) {
			s = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Conversion depuis TEI_CORPO</title></head><body>';
	} else {
			s = '<!DOCTYPE html><html><head><meta charset="UTF-8"><style>td {vertical-align:top;}</style></head><body><table>';
	}
	if (format.indexOf(';overlap;') >= 0)
		...
		if (format.indexOf(';header;') >= 0) {
			...
			var personDone = {};
			for (var d in p) {
				if (personDone[p[d].Name]) continue;
				personDone[p[d].Name] = true;
				s += '<p>' + '[person]';
